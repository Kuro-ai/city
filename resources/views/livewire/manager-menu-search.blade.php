<div>
    <div class="flex justify-between">
        ...
        <x-create-button href="{{ route('manager.menus.create') }}">
            New Menu
        </x-create-button>
    </div>
    ...
    <div class="container w-full px-5 py-6 mx-auto">
                            ...
                            <div class="w-[23rem] mx-4 mb-2 rounded-lg shadow-lg menu-item border-2 border-pale"
                                ...
                                <img class="w-full h-48" src="{{ asset('menus/' . $menu->image) }}"
                                    ...
                                    <div class="px-6 py-4 flex flex-col items-center justify-center">  
                                        <form action="{{ route('manager.order.addToCart') }}" method="post" class="text-center">
                                            @csrf
                                ...
                                <div class="flex space-x-2 justify-center items-center">
                                    <a href="{{ route('manager.menus.edit', $menu->id) }}"
        ...
        <div class="col-span-4 mb-4 category-header text-center text-pale mt-4">
            @if (empty($cartItems))
            <a href="{{ route('manager.order.shoppingcart', ['reservation_id' => session()->get('reservation_id')]) }}"
            ...
            @else
                <a href="{{ route('manager.order.shoppingcart', ['reservation_id' => $reservation_id]) }}"
                    class="bg-gradient-to-r from-bbyellow via-yellow-300 to-yellow-500 hover:bg-gradient-to-br text-bgcyan font-bold py-2 px-4 mt-4 rounded mx-2">Shopping Cart</a>
            @endif
        </div>
...
<script>
    ...
    function deleteItem(menuId) {
        if (confirm('Are you sure you want to delete?')) {
            ...
            form.method = 'POST';
            form.action = '{{ route('manager.menus.destroy', 'placeholder') }}'.replace('placeholder', menuId);
