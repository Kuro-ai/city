<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-bbyellow leading-tight">
            {{ __('Shopping Cart') }}
            <a href="{{ route('manager.order.shoppingcart', ['reservation_id' => request('reservation_id')]) }}"
    ...
    <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
        <div class="mt-6 sm:mt-8 md:gap-6 lg:flex lg:items-start xl:gap-8">
            <div class="mx-auto w-full flex-none lg:max-w-2xl xl:max-w-4xl">
                <div class="space-y-6">
                    <form action="{{ route('manager.order.updateCart') }}" method="post">
                        @csrf
                        ...
                        <div class="flex justify-center">
                            @if (empty($cartItems))
                                <a href="{{ route('manager.menus.index', ['reservation_id' => session()->get('reservation_id')]) }}"
                            ...
                            @else
                                <a href="{{ route('manager.menus.index', ['reservation_id' => $reservation_id]) }}"
                                    class="bg-gradient-to-r from-bbyellow via-yellow-300 to-yellow-500 hover:bg-gradient-to-br text-bgcyan font-bold py-2 px-4 mt-4 rounded mx-2">Menu</a>
                            @endif
                                </form>
                            <form action="{{ route('manager.order.clearCart') }}" method="post" class="mx-2">
                                @csrf
                                <button type="submit"
                    ...
                    <div class="flex justify-center">
                        <form action="{{ route('manager.order.cartToCheckout') }}" method="post" class="mx-2">
                            @csrf
                            <input type="hidden" name="reservation_id" value="{{ session()->get('reservation_id') }}">
