<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 ">
            ...
            <form class="max-w-sm mx-auto bg-bgcyan border-2 border-pale p-6 rounded-md" action="{{ route('manager.menus.store') }}"
                method="POST" enctype="multipart/form-data">
                @csrf
                <div class="flex justify-end">
                    <x-create-button href="{{ route('manager.menus.index') }}">
                        Menu List
                    </x-create-button>
                </div>
                <div class="mb-5">
                ...
                <div class="mb-5">
                    <label for="price" class="block mb-2 text-sm font-medium text-pale">New
                        Price</label>
                    <input type="number" id="price"
                        class="shadow-sm bg-bgcyan border-2 border-pale text-pale text-sm rounded-lg block w-full p-2.5 @error('price') border-red-600 @enderror"
                        placeholder="Price" name="price" min="0" max="99999999.99" step="0.01" />
                    @error('price')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
